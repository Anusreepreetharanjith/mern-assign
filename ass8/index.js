...
app.get('/book/delete/:id', (request, response) => {
    ...
    console.log('deleting the book', request.params);
...
app.get('/book/delete/yes/:id', (request, response) => {
    ...
    fs.readFile(dbPath, (err, data) => {
        ...
        let booksann = JSON.parse(data);
        booksann.forEach((element, i) => {
            if (element._id === id)
                booksann.splice(i, 1);
        });

        console.log(booksann.length);
        fs.writeFile(dbPath, JSON.stringify(booksann),(error) => {
            if (error) {
